...
export interface BrainRequest {
  ...
  localEndpoint?: string
  localModel?: string
  /** Which engine to use (passed from project.defaults.brain). */
...
export interface SlateApi {
  ...
  /** Windows host: re-probe Claude/Codex/local after menu login */
  onBrainRefresh?(cb: () => void): () => void
  /** Windows/Wails host: local file → data URL for <img> */
  fileAsDataURL?(path: string): Promise<string>
  appVersion?(): Promise<string>
  brainRunWith?(req: BrainRequest, backend: BrainBackend): Promise<BrainResult>
}

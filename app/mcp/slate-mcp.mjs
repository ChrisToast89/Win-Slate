...
//
// Tool catalog matches the in-app control server (projects + brain).
...
const TOOLS = [
  ...
  { name: 'brain_status', description: 'Report which brains are available: Claude Code, Codex, and any local model server.', inputSchema: { type: 'object', properties: {} } },
  { name: 'list_local_models', description: 'List models on the local OpenAI-compatible server.', inputSchema: { type: 'object', properties: { endpoint: { type: 'string' } } } },
  { name: 'set_brain', description: 'Set a project brain: claude | codex | local.', inputSchema: { type: 'object', properties: { projectId: { type: 'string' }, brain: { type: 'string' }, endpoint: { type: 'string' }, model: { type: 'string' } }, required: ['projectId', 'brain'] } }
]
  ...
  try {
    if (method === 'initialize') {
      reply(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'slate', version: '0.3.2-win.1' }
      })
